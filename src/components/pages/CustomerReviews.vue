    ...
    <div class="flex flex-col lg:flex-row gap-6 justify-center">
      <div v-for="review in displayedReviews" :key="review.id"
        ...
        <div class="flex items-center mb-2">
          <span class="text-yellow-400 text-lg mr-2">{{ review.rating }}</span>
          <i class="fa-solid fa-star text-yellow-400" v-for="n in Math.floor(review.rating)" :key="n"></i>
          <i v-if="review.rating % 1 !== 0" class="fa-solid fa-star-half-stroke text-yellow-400"></i>
        </div>
        <p class="text-sm mb-4">{{ review.comment }}</p>
        <div class="flex items-center gap-2">
          <img :src="review.userImage" alt="user" class="w-8 h-8 rounded-full object-cover" />
          <div>
            <div class="font-semibold text-xs">{{ review.userName }}</div>
            <div class="text-xs text-[var(--Color-Text-Text-Secondary)]">{{ review.date }}</div>
          </div>
        </div>
      </div>
    </div>
    <CarouselControls @prev="prevReview" @next="nextReview" />
  </div>
</template>
