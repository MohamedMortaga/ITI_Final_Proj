...
export const useEnhancedChat = (chatRoomId) => {
  // Reactive state
  const messages = ref([])
  ...
  /**
   * Set up real-time messages listener with enhanced features
   */
    ...
    messagesUnsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        if (messages.value.length > MESSAGES_LIMIT) {
          messages.value = messages.value.slice(-MESSAGES_LIMIT)
        }
  ...
  const setupTypingListener = () => {
    const typingRef = collection(db, 'user-chats', chatRoomId, 'typing')
  ...
  const sendMessage = async (content, type = 'text', attachment = null) => {
    ...
    if (content.length > MAX_MESSAGE_LENGTH) {
      showError('Message Too Long', `Message cannot exceed ${MAX_MESSAGE_LENGTH} characters`)
      return null
    }
    ...
    try {
      ...
      const messageData = {
        ...
        senderImage: currentUser.value.photoURL || null,
  ...
  const updateMessageStatus = async (messageId, status) => {
    try {
      const messageRef = doc(db, 'user-chats', chatRoomId, 'messages', messageId)
  ...
  const markAllAsRead = async () => {
    ...
    try {
      const unreadMessages = messages.value.filter(
        message => 
          message.senderId !== currentUser.value.uid && 
  ...
  const searchMessages = (query) => {
    if (!query.trim()) return messages.value
  ...
  const filteredMessages = computed(() => {
    ...
    return searchMessages(searchQuery.value)
  })
   ...
   */
  const cleanup = () => {
    console.log('🧹 Cleaning up enhanced chat system...')
    ...
    console.log('✅ Enhanced chat system cleaned up')
  }
