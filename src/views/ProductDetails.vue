<template>
  <div
    ...
    <div v-if="product"
      ...
      <div class="md:w-1/2">
        <img :src="product.img || require('@/assets/logo.png')" alt="Product Image"
          class="w-full h-auto rounded-lg object-cover mb-4" />
        <div class="bg-[var(--color-gray-100)] p-4 rounded-lg dark:bg-[var(--color-gray-700)]">
      ...
      <div class="md:w-1/2 flex flex-col gap-4">
        <div>
          <h1 class="text-2xl font-bold text-[var(--color-success-500)] dark:text-[var(--color-success-300)] mb-2">
            {{ product.title }}
          </h1>
          <p class="text-[var(--color-gray-600)] mb-2 dark:text-[var(--color-gray-400)]">
            <span
              class="font-semibold text-[var(--color-gray-800)] dark:text-[var(--color-gray-200)]">Description:</span>
            {{ product.details }}
          </p>
          <div class="bg-[var(--color-gray-100)] p-4 rounded-lg dark:bg-[var(--color-gray-700)]">
            <p class="text-[var(--color-gray-700)] dark:text-[var(--color-gray-300)]">
              <span
                class="font-semibold text-[var(--color-gray-800)] dark:text-[var(--color-gray-200)]">Category:</span>
              {{ product.category || product.type }}
            </p>
            <p class="text-[var(--color-gray-700)] dark:text-[var(--color-gray-300)]">
              <span class="font-semibold text-[var(--color-gray-800)] dark:text-[var(--color-gray-200)]">Model:</span>
              {{ product.model || "GSR 18V-50 Professional" }}
            </p>
            <p class="text-[var(--color-success-500)] font-bold dark:text-[var(--color-success-300)]">
              <span class="font-semibold text-[var(--color-gray-800)] dark:text-[var(--color-gray-200)]">Price per
                day:</span>
        ...
        <div class="bg-[var(--color-gray-100)] p-4 rounded-lg dark:bg-[var(--color-gray-700)]">
          ...
          <div class="flex items-center gap-4 mt-2">
            <img src="https://via.placeholder.com/40" alt="Owner" class="w-10 h-10 rounded-full" />
            <div>
              <p class="text-[var(--color-gray-800)] dark:text-[var(--color-gray-200)]">
                {{ booking.sellerName || "Loading..." }}
              </p>
              <p class="text-sm text-[var(--color-gray-500)] dark:text-[var(--color-gray-400)]">
                Member since March 2024
              </p>
              <p class="text-sm text-[var(--color-success-500)] dark:text-[var(--color-success-300)]">
                55 successful rentals
              </p>
              <button class="mt-1 text-[var(--color-success-500)] text-sm dark:text-[var(--color-success-300)]">
                Send a message
              </button>
            </div>
          </div>
    ...
    <!-- Booking Form Modal -->
    <div v-if="showBookingForm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div
        ...
        <h2 class="text-xl font-bold text-[var(--color-success-500)] dark:text-[var(--color-success-300)] mb-4">
          Book This Product
        </h2>
        <form @submit.prevent="submitBooking" class="space-y-4">
          <div>
            <label class="block text-[var(--color-gray-700)] dark:text-[var(--color-gray-300)]">Select Location on
              Map</label>
            <iframe
            ...
            <input v-model="booking.deliveryAddress" type="text"
            ...
            <p v-if="booking.deliveryMethod === 'delivery'"
          ...
          <div>
            <label class="block text-[var(--color-gray-700)] dark:text-[var(--color-gray-300)]">Delivery Method</label>
            <select v-model="booking.deliveryMethod"
              class="w-full p-2 rounded-lg bg-[var(--color-gray-100)] dark:bg-[var(--color-gray-700)] border border-[var(--color-success-200)]"
              required @change="updateDeliveryFee">
              <option value="delivery">Delivery</option>
              <option value="pickup">Pickup</option>
            </select>
          </div>
          <div>
            <label class="block text-[var(--color-gray-700)] dark:text-[var(--color-gray-300)]">Start Date</label>
            <p
              class="w-full p-2 rounded-lg bg-[var(--color-gray-100)] dark:bg-[var(--color-gray-700)] border border-[var(--color-success-200)]">
          ...
          <div>
            <label class="block text-[var(--color-gray-700)] dark:text-[var(--color-gray-300)]">End Date</label>
            <p
              class="w-full p-2 rounded-lg bg-[var(--color-gray-100)] dark:bg-[var(--color-gray-700)] border border-[var(--color-success-200)]">
          ...
          <div class="flex justify-end gap-4">
            <button type="button" @click="showBookingForm = false"
    ...
    <div v-if="showWebsiteReviewForm"
      class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-[var(--color-gray-25)] dark:bg-[var(--color-gray-800)] p-6 rounded-lg shadow-xl w-full max-w-md">
        <button @click="showWebsiteReviewForm = false"
          class="absolute top-2 right-2 text-[var(--color-gray-700)] dark:text-[var(--color-gray-300)] hover:text-[var(--color-gray-800)] dark:hover:text-[var(--color-gray-200)] text-2xl font-bold">
          ×
        </button>
        <h2 class="text-xl font-bold text-[var(--color-success-500)] dark:text-[var(--color-success-300)] mb-4">
        ...
        <form @submit.prevent="submitWebsiteReview" class="space-y-4">
          <div>
            <label class="block text-[var(--color-gray-700)] dark:text-[var(--color-gray-300)]">Review</label>
            <textarea v-model="newWebsiteReview.review"
              class="w-full p-2 rounded-lg bg-[var(--color-gray-100)] dark:bg-[var(--color-gray-700)] border border-[var(--color-success-200)]"
              placeholder="Write your review about our website" required></textarea>
          </div>
          <div>
            <label class="block text-[var(--color-gray-700)] dark:text-[var(--color-gray-300)]">Rate (1 - 5)</label>
            <input v-model.number="newWebsiteReview.rate" type="range" min="1" max="5" step="1"
              class="w-full p-2 rounded-lg bg-[var(--color-gray-100)] dark:bg-[var(--color-gray-700)] border border-[var(--color-success-200)]"
              required />
            <span class="ml-2">{{ newWebsiteReview.rate }} ★★★★★</span>
          </div>
          <div class="flex justify-end gap-4">
            <button type="button" @click="showWebsiteReviewForm = false"
              class="bg-[var(--color-gray-400)] text-[var(--color-gray-25)] py-2 px-4 rounded-lg hover:bg-[var(--color-gray-500)]">
              Cancel
            </button>
            <button type="submit"
    ...
    <div v-if="product"
      ...
      <h3 class="text-lg font-semibold text-[var(--color-gray-800)] mb-4 dark:text-[var(--color-gray-200)]">
        Tool reviews
      </h3>
      <div class="space-y-4">
        <div v-for="review in reviews" :key="review.id" class="p-4 bg-white rounded-lg shadow">
          <p class="text-[var(--color-gray-700)] dark:text-[var(--color-gray-300)]">
            "{{ review.review }}" - {{ review.userName || review.rentUserId }}
          </p>
          <div class="flex items-center gap-1">
            <!-- Full stars -->
            <span v-for="i in Math.floor(review.rate)" :key="'full-' + review.id + '-' + i"
              class="inline-block w-4 h-4 text-[var(--color-warning-500)]">
              ★
            </span>
            <!-- Empty stars -->
            <span v-for="i in 5 - Math.floor(review.rate)" :key="'empty-' + review.id + '-' + i"
              class="inline-block w-4 h-4 text-[var(--color-gray-400)]">
              ★
            </span>
            <span class="text-[var(--color-gray-600)] dark:text-[var(--color-gray-400)] ml-2">{{ review.rate }}</span>
          </div>
        </div>
      </div>
    ...
    <div v-if="showReviewForm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-[var(--color-gray-25)] dark:bg-[var(--color-gray-800)] p-6 rounded-lg shadow-xl w-full max-w-md">
        ...
        <form @submit.prevent="submitReview" class="space-y-4">
          <div>
            <label class="block text-[var(--color-gray-700)] dark:text-[var(--color-gray-300)]">Review</label>
            <textarea v-model="newReview.review"
              class="w-full p-2 rounded-lg bg-[var(--color-gray-100)] dark:bg-[var(--color-gray-700)] border border-[var(--color-success-200)]"
              placeholder="Write your review" required></textarea>
          </div>
          <div>
            <label class="block text-[var(--color-gray-700)] dark:text-[var(--color-gray-300)]">Rate (1 - 5)</label>
            <input v-model.number="newReview.rate" type="range" min="1" max="5" step="1"
              class="w-full p-2 rounded-lg bg-[var(--color-gray-100)] dark:bg-[var(--color-gray-700)] border border-[var(--color-success-200)]"
              required />
    ...
    <div v-if="product"
      ...
      <div class="grid grid-cols-2 gap-4">
        <div class="bg-[var(--color-gray-100)] p-4 rounded-lg dark:bg-[var(--color-gray-700)]">
          <img src="https://via.placeholder.com/100" alt="Camera" class="w-full h-24 object-cover rounded-lg" />
      ...
      <button class="mt-4 text-[var(--color-success-500)] text-sm dark:text-[var(--color-success-300)]">
        view all items
      </button>
    </div>

    <div v-else class="text-center text-[var(--color-gray-500)] py-10 text-lg dark:text-[var(--color-gray-400)]">
      Loading product...
    </div>
    <router-link to="/home"
